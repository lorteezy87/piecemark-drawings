...
const nav: NavItem[] = [
  ...
  { to: "/portfolio", label: "Portfolio — All Jobs", icon: Building2 },
  {
    to: "/tasks",
    label: "Task List",
    icon: ListChecks,
  ...
  { to: "/lookahead", label: "48h / 10-Day Look Ahead", icon: CalendarClock },
  { to: "/deliveries", label: "Deliveries", icon: Truck },
  { to: "/work-packages", label: "Work Packages", icon: PackageCheck },
  { to: "/change-orders", label: "Change Orders", icon: DollarSign },
  { to: "/roadblocks", label: "Roadblocks", icon: OctagonAlert },
  { to: "/drawings", label: "Drawing Sets", icon: FileStack, section: "Drawings control" },
  ...
  { to: "/holds", label: "Sheet Holds", icon: OctagonAlert },
  { to: "/projects", label: "Jobs", icon: Building2, section: "Setup" },
...
export function AppShell({
  ...
  useEffect(() => {
    if (!user) return;
    const name = user.displayName || user.primaryEmail?.split("@")[0] || "Station";
  ...
  const openHolds = useMemo(
    () => drawings.filter((d) => d.projectId === project?.id && d.status === "on_hold").length,
    [drawings, project?.id],
  );
  const openRfis = useMemo(
    () => rfis.filter((r) => r.projectId === project?.id && r.status === "open").length,
    [rfis, project?.id],
  );
  const overdueTasks = useMemo(() => {
    ...
    return tasks.filter((t) => {
      if (t.projectId !== project?.id || t.status === "done" || !t.dueDate) return false;
          ...
          <aside className="hidden w-64 shrink-0 flex-col border-r border-[var(--color-border)] bg-[var(--color-bg-elevated)] lg:flex">
            <div className="border-b border-[var(--color-border)] px-5 py-5">
              <Link to="/" className="block">
                ...
                <div className="mt-0.5 text-xs text-[var(--color-muted)]">Drawings control</div>
            ...
            <div className="space-y-2 border-b border-[var(--color-border)] px-4 py-4">
              ...
              {project && (
                <div className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface)] p-3 text-xs text-[var(--color-muted)]">
                  <div className="font-mono-num text-[var(--color-fg)]">{project.jobNumber}</div>
                  <div className="mt-1 line-clamp-2">{project.client}</div>
                  <div className="mt-2 flex gap-3 text-[11px]">
                    <span className="text-[var(--color-warn)]">{openHolds} holds</span>
                    <span className="text-[var(--color-info)]">{openRfis} open RFIs</span>
                    <span
                      className={
                        overdueTasks > 0
            ...
            <nav className="flex-1 space-y-0.5 overflow-y-auto p-3">
              {nav.map((item) => {
                ...
                return (
                  <div key={item.to}>
                    {item.section && (
                      <div className="px-3 pb-1 pt-3 text-[10px] font-medium uppercase tracking-[0.16em] text-[var(--color-subtle)]">
                        {item.section}
                      </div>
                    )}
                    <Link
            ...
            <div className="absolute inset-y-0 left-0 flex w-[min(100%,20rem)] flex-col bg-[var(--color-bg-elevated)] shadow-xl">
              <div className="flex items-center justify-between border-b border-[var(--color-border)] px-4 py-4">
                <div>
                  ...
                  <div className="text-xs text-[var(--color-muted)]">Drawings control</div>
                </div>
